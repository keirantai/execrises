...
$item_per_buckets = 7; // number of items render in a bucket
$number_of_buckets = 5; // number of buckets to render the items

$total_num_links = 26; // total number of items
$list = []; // list of items to render
for ($i=0; $i < $total_num_links; $i++) {
	$index = $i+1;
...
function main() {
	global $total_num_links, $list, $number_of_buckets, $item_per_buckets;

	$max_number_of_links = min($total_num_links, $item_per_buckets * $number_of_buckets);
	...
	for($i=0; $i < $max_number_of_links; $i++) {
		...
		for ($col=1; $col <= $number_of_buckets; $col++) {
			$index_where_to_break = $col * $item_per_buckets;
